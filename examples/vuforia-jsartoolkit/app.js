...
// disable jsartoolkit startup initially
var jsartoolkitSystem = arScene.systems["jsartoolkit"];
if (jsartoolkitSystem) {
    jsartoolkitSystem.disableStartup = true;
}
...
arScene.addEventListener('argon-vuforia-not-available', function(evt) {
    statusMsg.innerHTML = "vuforia not available, starting jsartoolkit...";
    if (jsartoolkitSystem) {
        jsartoolkitSystem.disableStartup = false;
        jsartoolkitSystem.startJSARToolKit();
        content.setAttribute("scale", {x: 10, y: 10, z: 10}); // TODO: remove this once jsartoolkit markers can be scaled correctly
        hudElem.innerHTML = 'Look at the "hiro" marker ...';
    }
});
...
arScene.addEventListener('argon-jsartoolkit-marker-load-failed', function(evt) {
    statusMsg.innerHTML = "jsartoolkit failed to load: " + evt.detail.error.message;
});
